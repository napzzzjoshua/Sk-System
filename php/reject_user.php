...
if ($user_data) {
    // The recipient_id for the notification is the ID of the user being rejected
    ...
    try {
        // 2. Update user status to 'Reject' in the correct source table
        $update_sql = "UPDATE {$source_table} SET status = 'Reject' WHERE id = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param("i", $user_id_to_act);
        $update_stmt->execute();
        $update_stmt->close();

        // 3. Insert notification into the 'notifications' table for the rejected user
        $message = "Your account was **rejected** by Admin {$admin_fullname}. Please re-submit your registration with correct details or contact support.";
        $link = "manage_users.php"; // Link to the user management page
        $icon = "fas fa-user-times";
        $color = "red"; // Custom color for rejection
        ...
        $_SESSION['notification_message'] = "User **{$user_fullname}** (Source: **{$source_table}**) has been successfully rejected and notified.";
        ...
        $_SESSION['notification_message'] = "Error rejecting user in table **{$source_table}**. Please try again.";
